...
const ERPAnalyzer = require('./analyzers/erpAnalyzer.js');
const { validateEmail } = require('./emailValidator.js');
const cache = require('./cache.js');
const { searchWithPuppeteer } = require('./searchPuppeteer.js');
...
const PHONE_PATTERNS = [
  /(?:\+31|0031|0)[\s.-]?(?:\d[\s.-]?){8,9}/g,
  /(?:\+32|0032|0)[\s.-]?(?:\d[\s.-]?){8,9}/g,
  /(?:\+49|0049|0)[\s.-]?(?:\d[\s.-]?){9,10}/g,
  /\+\d{1,3}[\s.-]?\d{4,14}/g,
];
...
function extractCompanyInfo($, url) {
  ...
  // City: prefer schema.org addressLocality, fall back to NL/BE/DE postal code pattern
  let city = $('[itemprop="addressLocality"]').first().text().trim();
  if (!city) {
    const addressText = addressCandidates[0] ?? '';
    // NL/BE: "1234 AB Amsterdam" → city is last token after postal code
    const nlMatch = addressText.match(/\b\d{4}\s?[A-Z]{2}\s+([A-Za-z\s\-]{2,40})/);
    if (nlMatch) city = nlMatch[1].trim();
  }
  ...
  return {
    ...
    city: city.substring(0, 80) || null,
  };
}

function makeUniqueIdentifier({ companyName, address, domain }) {
  const base = [companyName, address].filter(Boolean).join('|') || domain || '';
  return base
    .toLowerCase()
    .normalize('NFKD')
    .replace(/[̀-ͯ]/g, '')
...
async function scrapeSite(url, { emailValidation = true, deepValidation = false, analyzer = ERPAnalyzer } = {}) {
  ...
  const analysis = analyzer.analyze({ text: allText, url, domain, extraData, emails, companyName });
  ...
  return {
    ...
    website: url,
    domain,
    uniqueIdentifier: makeUniqueIdentifier({ companyName, address, domain }),
    email: primaryEmail,
    ...
    status: analysis.analysis_data?.status ?? (primaryEmail ? 'verrijkt' : 'nieuw'),
    assessment: analysis.analysis_data?.assessment ?? null,
    assessmentReason: analysis.analysis_data?.assessmentReason ?? null,
    hasVacancies: analysis.analysis_data?.hasVacancies ?? (analysis.analysis_data?.vacanciesCount > 0) ?? false,
    jobTitles: analysis.analysis_data?.jobTitles ?? analysis.analysis_data?.vacancyTitles ?? [],
    vacancyLinks: [
      analysis.analysis_data?.jobPageUrl,
      ...(analysis.analysis_data?.vacancyLinks ?? []),
    ].filter(Boolean),
    signals: analysis.analysis_data?.signals ?? {},
    emailValid,
...
class ScraperEngine {
  ...
  async runQuery(querySpec, { sector, country, onDomainFound, onLeadFound, onLog, onError, onSearchProgress } = {}) {
    ...
    const urls = await searchDDG(querySpec.query, {
      maxResults: this.searchResultsPerQuery,
...
module.exports = { searchDDG, scrapeSite, ScraperEngine };
